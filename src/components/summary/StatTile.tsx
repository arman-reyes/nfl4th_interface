interface Props {
  ...
  benchmark?: string
  /**
   * Optional colour for the value, when the value's own sign carries meaning
   * the tile should show. Left unset the value stays in ordinary ink.
   */
  valueColor?: string
}

/** One headline number. Shared by the team profile and the quiz scorecard. */
export function StatTile({ label, value, note, benchmark, valueColor }: Props) {
  return (
    <div className="rounded-md border border-stone-200 bg-white p-3">
      ...
      <dd
        className={`tnum mt-1 text-2xl leading-none font-bold ${
