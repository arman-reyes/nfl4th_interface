...
import { actualChoice, modelChoice, wpForfeited, wpOf } from './decision'
 ...
 * recommendation carried — the team's standing going into the 4th down if they
 * play it right. Bands are 65/35, 85/15 and 95/5.
 */
export function gameState(play: Play): GameState {
  const best = wpOf(play, modelChoice(play))
  if (best === null) return 'in doubt'
  const edge = Math.abs(best - 0.5)
...
export function decisionImpact(play: Play): DecisionImpact | null {
  ...
  if (actual === null || actual === modelChoice(play)) return null
